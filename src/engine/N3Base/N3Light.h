...
public:
	...
	public:
		...
		void		InitPoint(int nLgtNumber, const D3DXVECTOR3& dvPos, const _D3DCOLORVALUE& ltColor, float fRange = 10000.0f, float fAttenuation = 0.5f)
		{
			this->Zero();
		...
		void		InitDirection(int nLgtNumber, const D3DXVECTOR3& dvDir, const _D3DCOLORVALUE& ltColor)
		{
			this->Zero();
		...
		void		InitSpot(int nLgtNumber, const D3DXVECTOR3& dvPos, const D3DXVECTOR3& dvDir, const _D3DCOLORVALUE& ltColor, float fTheta, float fPhi, float fRange = 10000.0f)
		{
			this->Zero();
			nNumber = nLgtNumber; // 라이트 번호..
			Type = D3DLIGHT_SPOT;
